...
/*   Updated: 2020/12/03 18:44:35 by hiantrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/sh.h"

void	help_print_error_parenthesis(char *line, int i, char c)
{
	int		save;
...
int		print_error_parenthesis(char *line, char c, int i)
{
	...
	else if (c == '{' & line[i + 1] == '\0')
	{
		...
		help_print_error_parenthesis(line, i, c);
	return (0);
}
...
int		checker_for_re_sub(char *str, int i)
{
	...
	if (checker_for_anyparent(tmp) == 0 || checker_for_anycurch(tmp) == 0
		|| check_count_and(tmp, 0, 0) == 0
	...
	free(tmp);
	return (half_checker_for_re_sub(str, i, i, 0));
}
