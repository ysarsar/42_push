...
/*   Updated: 2020/12/03 18:17:10 by hiantrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		search_for_pa(char *str, int i, char a, int s)
{
	char	c;
	char	*tmp;

	tmp = ft_strsub(str, 0, i);
	if (search_no_espace(tmp) == 1)
		return (return_s(tmp, s));
	free(tmp);
	while (str[i])
	{
		c = str[i];
