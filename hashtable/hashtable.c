...
/*   Updated: 2020/12/02 18:03:52 by hiantrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_hash(char **args, t_hash **h_table, t_process *process)
{
	int			len;
	...
	if (len > 1)
	{
		...
		{
			reset_hits(args, h_table, process);
			delete_hashtable(h_table, len);
		}
		else if (ft_strcmp(args[1], "-l") == 0)
			l_flag(h_table, args, process);
		else if (ft_strcmp(args[1], "-d") == 0)
			d_flag(h_table, args, process);
		else if (ft_strcmp(args[1], "-t") == 0)
			t_flag(h_table, args, process);
		else if (ft_strcmp(args[1], "-p") == 0)
			p_flag(h_table, args, process);
		else if (args[1][0] == '-')
			ft_hash_error(args[1], process);
	}
	else
