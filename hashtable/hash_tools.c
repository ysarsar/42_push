...
/*   Updated: 2020/12/02 18:02:41 by hiantrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/sh.h"

void		reset_hits(char **args, t_hash **h_table, t_process *process)
{
	t_hash		*hashtable;
	t_ht		*current;
	int			i;
...
void		aff_hashtable(t_hash **h_table, t_process *process)
{
	...
	while (++i < TABLE_SIZE)
	{
		...
		{
			if (!c)
				ft_putendl_fd("hits    command", process->output);
			ft_putstr_fd("   ", process->output);
			ft_putnbr_fd(current->hits, process->output);
			ft_putstr_fd("    ", process->output);
