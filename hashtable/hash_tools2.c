...
/*   Updated: 2020/12/02 17:47:23 by hiantrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		search_hash_mini(t_hash **h_table, char **args, t_process *process)
{
	int		i;
	int		slot;
	t_ht	*current;
	...
	while (args[++i])
	{
		...
		if (!current)
			l_flag_error(args[i], process);
		while (current)
		{
			if (ft_strcmp(args[i], current->key) == 0)
			{
				ft_putstr_fd(args[i], process->output);
...
void		del_hash(t_hash **h_table, char **args, t_process *process)
{
	t_hash	*hashtable;
	t_ht	*current;
	int		i;
	...
	while (args[++i])
	{
		...
		if (delete_list(current, &(hashtable->list[slot]), args[i]) == 0)
		{
			...
			ft_putstr_fd(args[i], process->errorput);
